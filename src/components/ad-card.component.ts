...
@Component({
  ...
  template: `
    <div class="h-full flex flex-col neo-raised bg-gradient-to-br from-gray-50 to-gray-100 rounded-3xl overflow-hidden neo-hover hover:-translate-y-1 transition-all duration-300 group relative">
      
      <!-- Top Right Actions Menu -->
      <div class="absolute top-3 right-3 z-30 opacity-0 group-hover:opacity-100 transition-opacity">
        <button (click)="onDelete.emit()" class="neo-flat bg-gray-100 text-gray-500 hover:text-red-500 p-2 rounded-xl transition-colors" title="Delete Ad">
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5" class="w-4 h-4">
      ...
      <!-- Image Area -->
      <div class="relative aspect-[3/4] neo-inset w-full overflow-hidden" style="background: linear-gradient(135deg, #d1d5db 0%, #e5e7eb 100%);">
        @if (ad().imageUrl) {
          ...
          <div class="absolute top-4 left-4 neo-flat bg-gray-100 pl-2 pr-3 py-1.5 rounded-2xl text-xs font-bold flex items-center gap-2 z-10">
             <div class="w-5 h-5 flex items-center justify-center">
                @switch (ad().platform) {
                  ...
                  @default {
                     <svg class="w-5 h-5 text-gray-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>
                  }
                }
             </div>
             <span class="uppercase tracking-wide text-gray-700">{{ ad().platform }}</span>
          </div>

          <!-- A/B Variant Badge -->
          @if (ad().variantName) {
            <div class="absolute top-4 left-32 neo-flat bg-gradient-to-br from-indigo-500 to-indigo-600 text-white px-2.5 py-1.5 rounded-xl text-xs font-bold z-10">
              {{ ad().variantName }}
            </div>
          }
          ...
          <!-- SEO SCORE BADGE -->
          @if (ad().seo) {
            <div class="absolute top-4 right-14 neo-flat bg-gray-100 px-2.5 py-1.5 rounded-xl text-xs font-bold flex items-center gap-2 z-10">
          ...
          <!-- Actions Overlay -->
          <div class="absolute inset-0 bg-gradient-to-t from-gray-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6 z-20">
            <div class="flex items-center justify-between translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
               <span class="text-white text-xs font-medium neo-flat bg-white/10 px-3 py-1.5 rounded-2xl">Imagen 4.0</span>
               <button (click)="downloadImage(ad().imageUrl!)" class="neo-flat bg-white/20 hover:bg-white text-white hover:text-indigo-600 p-2.5 rounded-xl transition-colors">
          ...
          <div class="absolute inset-0 flex flex-col items-center justify-center text-gray-500 p-6 text-center">
            <div class="w-16 h-16 rounded-2xl neo-flat bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center mb-4">
            ...
            <button (click)="onGenerateImage.emit()" class="text-indigo-600 text-sm font-bold neo-raised bg-gradient-to-br from-gray-100 to-gray-200 px-4 py-2 rounded-xl transition-all neo-hover">
              Generate Visual
            </button>
          </div>
        }
      ...
      <div class="flex-1 p-6 flex flex-col relative z-10">
        ...
        <!-- Toggle Content / SEO -->
        <div class="flex justify-center mb-4 neo-inset bg-gray-200 p-1 rounded-xl">
          <button (click)="activeTab.set('content')" [class]="'flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ' + (activeTab() === 'content' ? 'neo-flat bg-gradient-to-br from-gray-50 to-gray-100 text-gray-800' : 'text-gray-600 hover:text-gray-800')">Preview</button>
          <button (click)="activeTab.set('seo')" [class]="'flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ' + (activeTab() === 'seo' ? 'neo-flat bg-gradient-to-br from-gray-50 to-gray-100 text-indigo-600' : 'text-gray-600 hover:text-gray-800')">SEO Metadata</button>
        </div>

        @if (activeTab() === 'content') {
          <div class="mb-4 animate-fade-in">
            <div class="flex items-start justify-between gap-4 mb-2">
               <h3 class="text-lg font-bold text-gray-800 leading-snug line-clamp-2">{{ ad().headline }}</h3>
               <button (click)="copyToClipboard(ad().headline + '\\n\\n' + ad().body)" class="text-gray-400 hover:text-indigo-600 transition-colors shrink-0 neo-flat rounded-lg p-1.5" title="Copy Text">
            ...
            <p class="text-gray-600 text-sm leading-relaxed font-medium line-clamp-4">{{ ad().body }}</p>
          </div>
        } @else {
          <!-- SEO View -->
          <div class="mb-4 animate-fade-in space-y-3 overflow-y-auto max-h-[140px] custom-scrollbar pr-2">
            @if (ad().seo; as seo) {
               <div>
                 <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">Keywords</span>
                 <div class="flex flex-wrap gap-1">
                   @for (kw of seo.keywords; track kw) {
                     <span class="px-2 py-1 neo-flat bg-gray-100 text-gray-700 rounded-lg text-[10px] font-medium">{{ kw }}</span>
                   }
                 </div>
               </div>

               <div>
                 <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">Hashtags</span>
                 <div class="flex flex-wrap gap-1">
                   @for (tag of seo.hashtags; track tag) {
                     <span class="text-indigo-600 text-[10px] font-medium">#{{ tag.replace('#','') }}</span>
                   }
                 </div>
               </div>

               <div>
                 <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">Meta Description</span>
                 <p class="text-xs text-gray-700 leading-relaxed italic neo-inset bg-amber-50 p-2 rounded-lg">{{ seo.metaDescription }}</p>
               </div>
            } @else {
               <div class="text-center py-6 text-gray-400 text-xs">No SEO data available.</div>
            }
          </div>
        }
